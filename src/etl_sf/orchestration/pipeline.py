...
class ETLPipeline:
    ...
    def run(self, progress: ProgressCallback | None = None) -> dict:
        run_id = str(uuid.uuid4())
        self.audit.start_batch(run_id, self.env.name)
        self._emit(progress, {"stage": "BATCH_START", "run_id": run_id, "env": self.env.name})

        file_summary: dict[str, dict] = {}
        try:
            for mapping in self.file_mappings:
                matched = self.ingestor.list_files(mapping)
                self._emit(progress, {"stage": "MAPPING_START", "pattern": mapping.file_pattern, "matched": len(matched)})
                for file in matched:
                    try:
                        self._emit(progress, {"stage": "FILE_READ", "file": file.name})
                        ...
                        if self.env.runtime.dry_run:
                            self._emit(progress, {"stage": "DRY_RUN", "file": file.name, "rows": len(transformed)})
                        else:
                        ...
                        self._emit(progress, {"stage": "FILE_DONE", "file": file.name, **file_summary[file.name]})
                        ...
                        self._emit(progress, {"stage": "FILE_FAILED", "file": file.name, "error": str(exc)})

            self._emit(progress, {"stage": "SF_START", "objects": len(self.sf_mappings)})
            sf_summary = {} if self.env.runtime.dry_run else self.sf_loader.run(self.sf_mappings)
            self.audit.close_batch(run_id, "SUCCESS")
            output = {"run_id": run_id, "files": file_summary, "salesforce": sf_summary}
            self._emit(progress, {"stage": "BATCH_DONE", **output})
            return output
        except Exception as exc:
            self.audit.close_batch(run_id, "FAILED")
            self.logger.exception("batch-failed", extra={"run_id": run_id})
            self._emit(progress, {"stage": "BATCH_FAILED", "run_id": run_id, "error": str(exc)})
            raise
